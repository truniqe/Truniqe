...
async function init() {
  console.log('[Admin Profit] init() starting...');
  await initAuth();
  
  console.log('[Admin Profit] isLoggedIn:', isLoggedIn(), 'isAdmin:', isAdmin());

  if (!isLoggedIn()) { 
    console.log('[Admin Profit] Not logged in, redirecting to auth...');
    requireAuth(); 
    return; 
  }
  if (!isAdmin()) {
    console.warn('[Admin Profit] User is logged in but NOT admin.');
  ...
  console.log('[Admin Profit] Admin verified, loading page...');
