...
async function init() {
  console.log('[Admin Room Types] init() starting...');
  await initAuth();
  
  console.log('[Admin Room Types] isLoggedIn:', isLoggedIn(), 'isAdmin:', isAdmin());

  if (!isLoggedIn()) { 
    console.log('[Admin Room Types] Not logged in, redirecting to auth...');
    requireAuth(); 
    return; 
  }
  if (!isAdmin()) {
    console.warn('[Admin Room Types] User is logged in but NOT admin.');
    showToast('Admin Access Required', 'Your user account does not have admin permissions.', 'warning');
    const tbody = document.getElementById('rooms-tbody');
    if (tbody) tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;padding:48px">
  ...
  console.log('[Admin Room Types] Admin verified, loading page...');
