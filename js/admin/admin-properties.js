...
async function init() {
  console.log('[Admin Properties] init() starting...');
  await initAuth();
  
  console.log('[Admin Properties] isLoggedIn:', isLoggedIn(), 'isAdmin:', isAdmin());

  if (!isLoggedIn()) { 
    console.log('[Admin Properties] Not logged in, redirecting to auth...');
    requireAuth(); 
    return; 
  }
  if (!isAdmin()) {
    console.warn('[Admin Properties] User is logged in but NOT admin.');
  ...
  console.log('[Admin Properties] Admin verified, loading page...');
