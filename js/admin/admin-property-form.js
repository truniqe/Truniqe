...
async function init() {
  console.log('[Admin Property Form] init() starting...');
  await initAuth();
  
  console.log('[Admin Property Form] isLoggedIn:', isLoggedIn(), 'isAdmin:', isAdmin());

  if (!isLoggedIn()) { 
    console.log('[Admin Property Form] Not logged in, redirecting to auth...');
    requireAuth(); 
    return; 
  }
  if (!isAdmin()) {
    console.warn('[Admin Property Form] User is logged in but NOT admin.');
    showToast('Admin Access Required', 'Your user account does not have admin permissions.', 'warning');
    const container = document.querySelector('.admin-content');
    if (container) container.innerHTML = `<div style="text-align:center;padding:48px">
      <strong>⚠️ Admin Access Required</strong><br>
      Your account does not have admin permissions.<br>
      <a href="/setup-admin.html" style="color:var(--gold-dark);font-weight:600;margin-top:8px;display:inline-block">Setup admin →</a><br>
      <a href="/debug-auth.html" style="color:var(--text-muted);font-size:13px;margin-top:4px;display:inline-block">Debug auth state →</a>
    </div>`;
    return;
  }
  
  console.log('[Admin Property Form] Admin verified, loading form...');
  setupSidebarToggle();
