...
async function init() {
  console.log('[Admin Calendar] init() starting...');
  await initAuth();
  
  console.log('[Admin Calendar] isLoggedIn:', isLoggedIn(), 'isAdmin:', isAdmin());

  if (!isLoggedIn()) { 
    console.log('[Admin Calendar] Not logged in, redirecting to auth...');
    requireAuth(); 
    return; 
  }
  if (!isAdmin()) {
    console.warn('[Admin Calendar] User is logged in but NOT admin.');
  ...
  console.log('[Admin Calendar] Admin verified, loading page...');
  setupSidebarToggle();
