...
/**
 * Fetch all properties for admin (includes owner fields + room type count)
 */
export async function fetchAllPropertiesAdmin() {
  const { data, error } = await supabase
    .from('properties')
    .select(`
      id, name, location, state, cover_image_url, status, angle_tags, created_at,
  ...
  return data.map(p => ({
    ...p,
    room_types_count: p.room_types?.length ?? 0,
    room_types: undefined,
  }));
}

/**
 * Fetch all room types across all properties (admin)
 */
export async function fetchAllRoomTypes() {
  const { data, error } = await supabase
    .from('room_types')
    .select('id, property_id, name, description, base_price, max_guests, sort_order, created_at')
    .order('created_at', { ascending: false });
  if (error) throw error;
  return data;
}
...
/**
 * Fetch all bookings (admin) — includes owner_phone/owner_name for WhatsApp
 */
